...
while Param_Q <1.01:
    ...
    # temp = (Ds^-1)^(1-q) * S * (Ds^-1)^q
    
    MatrixTemp = INV_Degree_Social_Qsub * MatrixAdjacency_Social * INV_Degree_Social_Q
    ...
    MatrixNet_W1 = np.dot(MatrixTemp,MatrixNet_W1)
    print " \nW1:"
...
#     print MatrixNet_W1
    ...
    MatrixNet_W2 = np.dot(MatrixTemp,MatrixNet_W2)
    print " \nW2:"
    print "MatrixNet_W2's is Shape :"+str(MatrixNet_W2.shape)
#     print MatrixNet_W2
    
    
    # W3 = (Du^-1)^(1-q) * A * (Di^-1) * B * (Du^-1)^q * (Ds^-1)^(1-q) * S * (Ds^-1) * S * (Ds^-1)^q
    
    MatrixNet_W3 = np.dot(np.dot(np.dot(np.dot(np.dot(np.dot(np.dot(np.dot(np.dot(INV_Degree_User_Qsub,MatrixAdjacency_UserItem),INV_Degree_Item),T_MatrixAdjacency_UserItem),INV_Degree_User_Q),INV_Degree_Social_Qsub),MatrixAdjacency_Social),INV_Degree_Social),MatrixAdjacency_Social),INV_Degree_Social_Q)
    
    Param_P = 0.0
    
    while Param_P < (1-Param_Q):
        Param_M = 0.0 
        
        while Param_M < (1-Param_Q - Param_P):
        
            print '*********'
            print 'Param_Q is '+str(Param_Q)
            print '    Param_P is '+str(Param_P)
            print '        Param_M is '+str(Param_M)
        
            MatrixNet = Param_P * MatrixNet_W1 + Param_Q * MatrixNet_W2 + (1-Param_P - Param_Q)
